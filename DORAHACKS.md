...
[faktura.axiqo.xyz](https://faktura.axiqo.xyz), click **▶ Use the live AI
desk · 3–6 min** and pick a guided walkthrough. You trigger each step yourself: the
...
- **Policy firewall** (fastest proof) — the AI _approves_ an invoice sized
...
**Final interaction build: tag `v0.2.4-final` — the exact runtime commit is
printed LIVE in `/api/judge/health → release.gitSha`, `/api/meta` and the
page-footer Build chip (pin any observed behavior to that SHA).**
Interaction regression verified end-to-end on production: minimize →
homepage pill → resume · back-to-picker keeps the run resumable · refresh
mid-transaction reattaches to the still-settling step (live phase notes +
early explorer link stream in while finality is pending) · abandon pre-fund
releases the reservation · abandon after funding hands off to the cleanup
worker · **sessions are per-visitor** — several judges hold independent
walkthroughs at once; only the on-chain SIGNATURE is serialized, and a
collision answers retry-soon (the page queues and retries by itself) ·
idle walkthroughs expire after 40 minutes (a mid-flight transaction is
never expired or evicted) · x402 charges survive a backend restart · the
same paid proof re-delivers its report idempotently (a different nonce
reusing the deploy is refused).
