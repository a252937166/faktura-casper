...
const INTAKES = [
  {
    ...
    description:
      "March freight services, 14 pallet shipments Hamburg to Vienna, delivered and signed.",
...
async function main() {
  ...
  console.log(
    `pool liquid ${Number(BigInt(before.liquid) / 1_000_000_000n)} CSPR, invoices ${before.invoiceCount}`,
  );
  ...
  for (const intake of INTAKES) {
    ...
    console.log(
      `  decision: ${r.status.toUpperCase()}  risk=${r.decision?.riskScore}  discount=${r.decision?.discountBps}bps  (${((Date.now() - t) / 1000).toFixed(0)}s)`,
    );
    console.log(`  rationale: ${r.decision?.rationale}`);
    if (r.decision?.redFlags?.length)
      console.log(`  red flags: ${r.decision.redFlags.join(" | ")}`);
